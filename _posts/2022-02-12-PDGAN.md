...
## Generator - Diversity(pre-learnt DPP) 학습 방법
...
각 사용자마다 generator가 만든 $T_{\mathcal{G}}$의 relevance를 평가하는 것이 discriminator의 역할입니다.<br>
output은 [0,1]이며 값이 클수록 relevance가 높다고 알려주는 일종의 score를 의미합니다. <br>
...
## Optimization
최종적으로 objective는 다음과 같이 정의됩니다.
- $\min \sum_{u=1}^{M} \log (1-\mathcal{D}(u,\mathcal{G}))$

중요한 점은, sampling이 순차적으로 진행되기 때문에 강화학습 방식을 사용해서 진행해야 하며 objective를 수정하면 다음과 같습니다. 
$\max \mathcal{J}(T_{\matchcal{G}}) = \sum_{i=1}^{K} \mathcal{R}(u,T_{\matchcal{G}}) \log \mathcal{Q}(u, i)$
- $\mathcal{Q}(u, i)$는 posterior probability를 의미합니다
- reward function $\mathcal{R}(u,T_{\matchcal{G}}) = -\log (1-\mathcal{D}(u,T_{\mathcal{G}}))
- 이렇게 정의할 경우 discriminator가 relevance score가 높다고 한 경우 reward는 더 증가하기 때문에 
