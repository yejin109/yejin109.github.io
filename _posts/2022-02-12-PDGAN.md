...
2. 다음 objective를 최적화 한다면 두번째 항을 통해서 유사한 아이템이 등장할 확률을 줄일 수 있습니다.
    - $ \max \mathcal{J} = \log \mathcal{P}(\matchcal{T}\mid A) - \log \mathcal{P}(\tilde{T}\mid A) $
    - $\max \mathcal{J} = \sum_{x=1}^{X} \log \mathcal{P}(\matchcal{T}_{x}\mid A) - \log \mathcal{P}(\tilde{T}_{x}\mid A) $
