...
이 때 MF를 이용하여 quality evaluating function을 정의하면 다음과 같습니다.
    - $\mathcal{Q}(u,i) = \sigma (v_{\mathcal{G}}^{u}*v_{\mathcal{G}}^{i}), v_{u}&v_{i}$ 는 $1\times D$ embedding vector이며 $\sigma$는 sigmoid function을 말합니다.
...
**$\mathcal{J}(T_{\matchcal{G}}) = \sum_{i=1}^{K} \mathcal{R}(u,T_{\matchcal{G}}) \log \mathcal{Q}(u, i)$**
