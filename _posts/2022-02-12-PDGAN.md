...
## Relevance 학습방법
pre-learnt DPP과 달리 Relevance를 합친 modified DPP는 다음과 같이 정의됩니다. 
$\mathcal{P}(T) \propto \sum_{i\in T}\mathcal{Q}(u,i) + (1-\alpha) \det(L_{T})$
    ...
    - 해당 형태가 MMR과 유사한 형태로 생각할 수 있을 것 같습니다.

이 때 MF를 이용하여 정의하면 다음과 같습니다.
    - $\mathcal{Q}(u,i) = \sigma (v_{u}v_{i}^{T}), v_{u}&v_{i}$ is $1\times D$ embedding vector